...
type callback('a) =
  ({. "nativeEvent": RN_Event.NativeEvent.t}, jsGestureState) => 'a;

let callback =
    (x: (RN_Event.NativeEvent.t, gestureState) => 'a): callback('a) =>
  (event, state) =>
    x(
      event##nativeEvent,
      {
...
let animatedEvent = l => {
  let config =
    List.fold_left(
      (x, y) =>
        switch (y) {
        | `X((value: RN_animatedRe.Value.t)) =>
          Js.Obj.assign(x, {"dx": value})
        | `Y((value: RN_animatedRe.Value.t)) =>
          Js.Obj.assign(x, {"dy": value})
        | `XY(value) =>
          Js.Obj.assign(
            x,
            RN_animatedRe.ValueXY.{"dx": getX(value), "dy": getY(value)},
          )
        },
      Js.Obj.empty(),
      l,
