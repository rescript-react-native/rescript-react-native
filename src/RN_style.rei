...
type pt_pct_animated('a) =
  | Pt(float)
  | Pct(float)
  | Animated(RN_animatedRe.value('a));
type float_animated('a) =
  | Float(float)
  | Animated(RN_animatedRe.value('a));
type string_interpolated =
  | String(string)
  | Animated(RN_animatedRe.Interpolation.t);
...
module Transform: {
  ...
  let makeAnimated:
    (
      ~perspective: RN_animatedRe.value('a)=?,
      ~rotate: RN_animatedRe.value('a)=?,
      ~rotateX: RN_animatedRe.value('a)=?,
      ~rotateY: RN_animatedRe.value('a)=?,
      ~rotateZ: RN_animatedRe.value('a)=?,
      ~scaleX: RN_animatedRe.value('a)=?,
      ~scaleY: RN_animatedRe.value('a)=?,
      ~translateX: RN_animatedRe.value('a)=?,
      ~translateY: RN_animatedRe.value('a)=?,
      ~skewX: RN_animatedRe.value('a)=?,
      ~skewY: RN_animatedRe.value('a)=?,
      unit
    ) =>
    styleElement;
};
